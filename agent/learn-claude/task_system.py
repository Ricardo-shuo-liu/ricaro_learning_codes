...
class FakeToolBlock:
    """为hook模拟block对象，适配原有permission_hook/log_hook"""
    def __init__(self, name: str, input: dict):
        self.name = name
        self.input = input


def permission_hook(block: FakeToolBlock):
    if block.name == "bash":
...
def log_hook(block: FakeToolBlock):
    preview = str(list(block.input.values())[:2])[:60]
    print(f"\033[90m[HOOK] {block.name}({preview})\033[0m")
    return None


def large_output_hook(block: FakeToolBlock, output):
...
def execute_tool(tool_name: str, tool_args: dict):
    block = FakeToolBlock(tool_name, tool_args)
...
if __name__ == "__main__":
    print("s10_openai: Task System - dependencies and task state")
