...
# 模拟 block 对象，适配 hook；OpenAI tool_call 转简易 dict‑like 对象
class FakeToolBlock:
    ...
    with cron_lock:
        for delivered in jobs:
            ...
            if current.recurring:
                ...
                scheduled_jobs.pop(current.id, None)
        try:
...
def execute_tool_openai(tool_call) -> tuple[str, str]:
    """接收 openai tool_call 对象，返回 tool_call_id, content"""
    name = tool_call.function.name
    args = _json.loads(tool_call.function.arguments)
    block = FakeToolBlock(name, args)
    blocked = trigger_hooks("PreToolUse", block)
    if blocked is not None:
        return tool_call.id, str(blocked)
    ...
    return tool_call.id, str(output)
...
def agent_loop(messages: list, context: dict | None = None):
    ...
    while True:
        ...
        for tc in msg.tool_calls:
            tid, out = execute_tool_openai(tc)
            messages.append({
                "role":"tool",
                "tool_call_id": tid,
                "content": out
...
if __name__ == "__main__":
    print("s12: Cron Scheduler (OpenAI‑compatible)")
