...
class FakeToolBlock:
    """Adapt an OpenAI tool call to the block shape used by hooks/handlers."""

    def __init__(self, tool_call):
        self.id = tool_call.id
        self.name = tool_call.function.name
        try:
            self.input = json.loads(tool_call.function.arguments or "{}")
        except json.JSONDecodeError:
            self.input = {}
...
def spawn_teammate_thread(name: str, role: str, prompt: str,
    ...
    def run_loop():
        ...
        sub_tools = [
            ...
            {"type": "function",
             "function": {"name": "edit_file", "description": "Replace text in a file.",
                          "parameters": {"type": "object",
                                         "properties": {
                                             "path": {"type": "string"},
            ...
            {"type": "function",
             "function": {"name": "glob", "description": "Find files by glob pattern; ** matches recursively.",
                          "parameters": {"type": "object",
                                         "properties": {
                                             "pattern": {"type": "string"}},
                                         "required": ["pattern"]}}},
        ...
        while not should_stop:
            ...
            summary = _last_assistant_text(message.content)
            gate = plan_gates.get(name, "not_required")
...
def permission_hook(block):
    # The permission layer sees the raw tool call before dispatch. It can deny,
    ...
    {"type": "function",
     "function": {"name": "write_file", "description": "Write content to a file.",
                  "parameters": {"type": "object",
                                 ...
                                 "required": ["path", "content"]}}},
    {"type": "function",
     "function": {"name": "edit_file", "description": "Replace exact text in a file once.",
                  "parameters": {"type": "object",
                                 "properties": {"path": {"type": "string"},
                                                "old_text": {"type": "string"},
                                                "new_text": {"type": "string"}},
                                 "required": ["path", "old_text", "new_text"]}}},
    {"type": "function",
     "function": {"name": "glob", "description": "Find files matching a glob pattern; ** matches recursively.",
                  "parameters": {"type": "object",
...
def has_tool_use(message) -> bool:
    # Do not rely on finish_reason alone; the concrete tool_calls field is the
...
def save_output(tool_call_id: str, output: str) -> Path:
...
def persisted_preview(tool_call_id: str, output: str,
...
def persist_large_output(tool_call_id: str, output: str) -> str:
    if len(output) <= PERSIST_THRESHOLD:
        return output
    return persisted_preview(tool_call_id, output)


def tool_result_budget(messages: list, max_bytes: int = 200_000) -> list:
    if not messages:
        return messages
    results = [msg for msg in messages if msg.get("role") == "tool"]
    total = sum(len(str(msg.get("content", ""))) for msg in results)
    if total <= max_bytes:
        return messages
...
def summarize_history(messages: list) -> str:
    ...
    response = client.chat.completions.create(
        ...
        max_tokens=2000)
    return extract_text(response.choices[0].message.content) or "(empty summary)"
...
# Slow tools return a placeholder tool message immediately. Their real output is
...
# The model sees OpenAI function schemas; Python executes handlers. S15 keeps
# both tables explicit so every added capability is visible in one place.
BUILTIN_TOOLS = [
    {"type": "function",
     "function": {"name": "bash", "description": "Run a shell command.",
                  "parameters": {"type": "object",
    ...
    {"type": "function",
     "function": {"name": "get_task", "description": "Get full task details.",
                  "parameters": {"type": "object",
                                 "properties": {"task_id": {"type": "string"}},
                                 "required": ["task_id"]}}},
    {"type": "function",
     "function": {"name": "claim_task", "description": "Claim a pending task.",
                  "parameters": {"type": "object",
                                 "properties": {"task_id": {"type": "string"}},
                                 "required": ["task_id"]}}},
    {"type": "function",
     "function": {"name": "complete_task", "description": "Complete an in-progress task.",
                  "parameters": {"type": "object",
                                 "properties": {"task_id": {"type": "string"}},
                                 "required": ["task_id"]}}},
    {"type": "function",
     "function": {"name": "schedule_cron",
    ...
    {"type": "function",
     "function": {"name": "list_crons", "description": "List registered cron jobs.",
                  "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function",
     "function": {"name": "cancel_cron", "description": "Cancel a cron job by ID.",
                  "parameters": {"type": "object",
                                 "properties": {"job_id": {"type": "string"}},
                                 "required": ["job_id"]}}},
    {"type": "function",
     "function": {"name": "spawn_teammate", "description": "Spawn a persistent teammate.",
                  "parameters": {"type": "object",
                                 "properties": {"name": {
                                                    "type": "string",
    ...
    {"type": "function",
     "function": {"name": "list_teammates", "description": "List active teammates.",
                  "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function",
...
def build_user_content(results: list[dict]) -> list[dict]:
    # OpenAI: each tool result is its own role="tool" message. Completed
    # background notifications follow as a separate user text message.
    content = list(results)
    for note in collect_background_results():
        content.append({"role": "user", "content": note})
    return content
...
def agent_loop(messages: list, context: dict, active_request: str):
    ...
    while True:
        ...
        # the model, execute tool calls, append tool messages, repeat.
        ...
        messages.extend(build_user_content(results))
